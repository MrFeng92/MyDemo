...
public class ValiImgServlet extends HttpServlet
{
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            String s = cs.charAt(getRand(0, cs.length() - 1)) + "";
            buf.append(s);
            g.setColor(new Color(getRand(0, 255), getRand(0, 255), getRand(0, 255)));
            g.setFont(new Font("宋体", Font.BOLD, 16));
            double theta = Math.PI / 180 * getRand(-45, 45);
            g.rotate(theta, 5 + base * i, 21);
            g.drawString(s, 5 + base * i, 21);
            g.rotate(theta * -1, 5 + base * i, 21);
        }
        System.out.println(buf.toString());
        // --向session中保存验证码，为后续的校验做准备
        request.getSession().setAttribute("valistr", buf.toString());
